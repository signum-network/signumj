...
import signumj.crypto.rs.ReedSolomon;
import signumj.crypto.rs.ReedSolomonImpl;
...
class SignumCryptoImpl extends AbstractSignumCrypto {

    static final SignumCryptoImpl INSTANCE = new SignumCryptoImpl();
    ...
    @Override
    public SignumID hashToId(byte[] hash) {
        if (hash == null || hash.length < 8) {
            throw new IllegalArgumentException("Invalid hash: " + Arrays.toString(hash));
        }
        return SignumID.fromLong(bytesToLongLE(hash));
    }
    ...
    @Override
    public EncryptedMessage encryptBytesMessage(byte[] message, byte[] myPrivateKey, byte[] theirPublicKey) {
        return encryptPlainMessage(message, false, myPrivateKey, theirPublicKey);
    }

    @Override
    public EncryptedMessage encryptTextMessage(String message, byte[] myPrivateKey, byte[] theirPublicKey) {
        return encryptPlainMessage(stringToBytes(message), true, myPrivateKey, theirPublicKey);
    }

    private EncryptedMessage encryptPlainMessage(byte[] message, boolean isText, byte[] myPrivateKey, byte[] theirPublicKey) {
        if (message.length == 0) {
            return new EncryptedMessage(new byte[0], new byte[0], isText);
        }
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            ...
            return new EncryptedMessage(data, nonce, isText);
        } catch (IOException e) {
            throw new RuntimeException(e.getMessage(), e);
        }
    }
    ...
    @Override
    public byte[] decryptMessage(EncryptedMessage message, byte[] myPrivateKey, byte[] theirPublicKey) {
    ...
    @Override
    public String rsEncode(SignumID burstID) {
        return reedSolomon.encode(burstID.getSignedLongId());
    }

    @Override
    public SignumID rsDecode(String rs) {
        rs = rs.toUpperCase();
        long rsValue;
        try {
        ...
        return SignumID.fromLong(rsValue);
    }
    ...
    @Override
    public byte[] getATCreationBytes(short atVersion, byte[] code, byte[] data, short dPages, short csPages, short usPages, SignumValue minActivationAmount) {
