...
abstract class AbstractSignumCrypto implements SignumCrypto {
    ...
    @Override
    public EncryptedMessage encryptBytesMessage(byte[] message, String myPassphrase, byte[] theirPublicKey) {
        return encryptBytesMessage(message, getPrivateKey(myPassphrase), theirPublicKey);
    }

    @Override
    public EncryptedMessage encryptTextMessage(String message, String myPassphrase, byte[] theirPublicKey) {
        return encryptTextMessage(message, getPrivateKey(myPassphrase), theirPublicKey);
    }

    @Override
    public byte[] decryptMessage(EncryptedMessage message, String myPassphrase, byte[] theirPublicKey) {
    ...
    @Override
    public byte[] calculateGenerationSignature(byte[] lastGenSig, SignumAddress lastGenerator) {
        return calculateGenerationSignature(lastGenSig, lastGenerator.getSignedLongId());
    }

    @Override
    public BigInteger calculateHit(SignumAddress accountId, long nonce, byte[] genSig, int scoop, int pocVersion) {
        return calculateHit(accountId.getSignedLongId(), nonce, genSig, scoop, pocVersion);
    }

    @Override
    public BigInteger calculateDeadline(SignumAddress accountId, long nonce, byte[] genSig, int scoop, long baseTarget, int pocVersion) {
