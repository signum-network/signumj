...
public abstract class PlotCalculatorTest {
    ...
    public void testPlotCalculatorCalculateDeadline() {
        BigInteger deadline = plotCalculator.calculateDeadline(TestVariables.EXAMPLE_ACCOUNT_ID.getSignedLongId(), 0, exampleGenSig, plotCalculator.calculateScoop(exampleGenSig, exampleHeight), exampleBaseTarget, exampleHeight);
        assertEquals(BigInteger.valueOf(76595641219705L), deadline);
    }

    @Test
    public void testPlotCalculatorCalculateHit() {
        assertEquals(new BigInteger("11245277265433361116"), plotCalculator.calculateHit(TestVariables.EXAMPLE_ACCOUNT_ID.getSignedLongId(), 0, exampleGenSig, 0, exampleHeight));
